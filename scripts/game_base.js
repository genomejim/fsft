...
//draw enemy health,shiend and ammo bars
                ...
                if (npcs[i].draw && npcs[i].role == 'enemy' && npcs[i].state != 'defeated' && npcs[i].beam != true){
                   ...
                   if (chars,jim.x < npcs[i].x  && npcs[i].ammo > 0){
                    ...
                    _canvasBufferContext.closePath();
                     ...
                     //TODO: this case broken - meant to be feedback when space is pressed and no enemies 
                     // are present - this canse now also fires when out of ammo
                    _canvasBufferContext.fillStyle    = '#fff';
                    _canvasBufferContext.fillText('No Targets Detected', chars.jim.x +50, chars.jim.y-50);
        }
}
//draw the razer beam
        for (var i in npcs) {
